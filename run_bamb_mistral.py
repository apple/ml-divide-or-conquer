...
openai.api_key = [el.strip() for el in open("../openai_newkey.txt", "r")][0]
serpapi_key = ""
global_path = "/mnt/task_wrapper/user_output/artifacts/"
global_path = "./"
...
def call_mistral(prompt, backbone):
    ...
    model = backbone[0]
    ans = model(messages, max_length=512)
    return ans[0]['generated_text'][1]['content']


def call_batch_mistral(prompts, backbone):
    import time
    a = time.time()
    model, tokenizer = backbone[0], backbone[1]
...
def vinilla_ask(context, question, backbone):
    if isinstance(backbone, str):
        ...
        ans = call_mistral(context + ' ' + question, backbone)
...
def vanilla_ask(context, question, subqs, backbone):
    if isinstance(backbone, str):
        cur_prompt = context + ' ' + question
        ans = call_gpt(cur_prompt, backbone)
    else:
...
def bamboogle_direct_qa(context, question, subqs, backbone):
    #prefix_prompt = '''Given the provided question, please provide a concise answer with no more than five words. If the answer contains a number, provide only the numerical value, without any accompanying text. Question: '''
    ...
    if isinstance(backbone, str):
        ...
        ret_text = call_mistral(cur_prompt.strip(), backbone)
    print(ret_text)
    #print('1' * 100)
    return ret_text
...
def cot(context, question, subqs, backbone):
    prefix_prompt = '''Given the provided context and question, please provide a concise answer with no more than five words. If the answer contains a number, provide only the numerical value, without any accompanying text. Question: '''
    cur_prompt = prefix_prompt + context + ' ' + question.strip() + " Let\'s think step by step."
    #cur_prompt = context + ' ' + question
    print(cur_prompt)
    if isinstance(backbone, str):
        ret_text = call_gpt(cur_prompt, backbone)
...
def bamboogle_direct_infer_0(question, subqs, backbone):
    #prefix_prompt = '''Your task is to tackle a complex question by first addressing a series of related subquestions, which will collectively guide you to solve the original question. The answer to the original question is supposed to be attached in the end in the format of \'The answer is: \'. Question: '''
    ...
    if subqs != '':
        cur_prompt = prefix_prompt + question.strip() + ' Subquestions: ' + ' '.join(subqs.split('\n'))
    else:
        cur_prompt = question.strip()
    ...
    if isinstance(backbone, str):
        ...
        ret_text = call_mistral(cur_prompt, backbone)
    print(ret_text)
...
def newgsm_cot_infer_0(question, subqs, backbone):
    #prefix_prompt = '''Your task is to tackle a complex question by first addressing a series of related subquestions, which will collectively guide you to solve the original question. The answer to the original question is supposed to be attached in the end in the format of \'The answer is: \'. Question: '''
    prefix_prompt = '''I want to solve a complex question by answering several related subquestions that would help me to answer it first. I want you to answer the subquestions one by one and finally solve the original question. The final answer is supposed to attached in the end in the format of \'The answer is: \'. Now comes with our real question and its subquestions: '''
    ...
    if isinstance(question, str):
        ...
        cur_prompt = [prefix_prompt + question[idx].strip() + ' Subquestions: ' + ' '.join(subqs[idx].split('\n')) for idx in range(len(question))]
...
def gsm_cot_infer_0(question, subqs, backbone):
    ...
    if isinstance(backbone, str):
        ...
        if isinstance(question, str):
            ...
            ret_text = call_batch_mistral(cur_prompt, backbone)
...
def vinilla_mistral(question, subqs, backbone):
    cur_prompt = '''I want to solve a complex question by answering several related subquestions that would help me to answer it first.
Now comes with our real question and its subquestions, I want you to answer the subquestions one by one and finally solve the original question: ''' + question.strip() + '\n' + subqs + '\n'
    ...
    with open('./datasets/gsm8k/test.jsonl') as inf:
        for line in inf:
            if count >= recover_num:
                ...
                pred = model(q, backbone).replace(',', '').strip()
                print('golden: ', a)
                print('pred: ', pred)
                print('*' * 50)
...
def gsm_cot_decompose(question, backbone):
    ...
    if isinstance(backbone, str):
        ...
        if isinstance(question, str):
            ret_text = call_mistral(cur_prompts, backbone)
        else:
            ret_text = call_batch_mistral(cur_prompts, backbone)
    return ret_text

def bamboogle_cot_decompose(question, backbone):
    #seed=0602
    ...
    #seed=0614
    #cur_prompt = '''Your task is to break down a given complex question into the most relevant and helpful subquestions, ensuring that no more than two subquestions are formulated for each question. Only return subquestions that directly aid in answering the original question, avoiding any that could be harmful or unhelpful.\nQuestion: '''
    #seed=0615
    #cur_prompt = '''Your task is to break down a given complex question into the two most helpful subquestions, ensuring that each subquestion directly aids in answering the original question, avoiding any that could be harmful or unhelpful.\nQuestion: '''
    #seed=0616, 0617
    #cur_prompt = '''Decompose below question into two subquestions that can help answer the original question: '''
    #pp = ' Only returns with the decomposed two subquestions without extra explaination.'
    #seed=0618
    #cur_prompt = '''Decompose below question into two concrete subquestions that can help answer the original question: '''
    #pp = ' Only returns with the decomposed two subquestions without extra explaination.'
    #seed=0619 0620
    cur_prompt = '''Decompose below question into two small subquestions that can help answer the original question: '''
    pp = ' Only returns with the decomposed two subquestions without extra explaination.'
    ...
    if isinstance(question, str):
        cur_prompts = cur_prompt + question.strip() + pp
    else:
        cur_prompts = [cur_prompt + q.strip() + pp for q in question]
...
def decompose_drop(model, modelname, datasetname, seed, backbone, subset=-1, whether_train=False, numerical_only=False, gpu_id=0, total_gpu=8):
    ...
    if not os.path.exists(global_path + 'golden-decomposing-by-' + backbone_name+'/'):
        os.mkdir(global_path + 'golden-decomposing-by-' + backbone_name+'/')  

    if whether_train:
        pathname = global_path + 'golden-decomposing-by-' + backbone_name+'/'+ph+'train-'+modelname+'_'+datasetname+'_'+str(gpu_id)+'-'+str(total_gpu)+'_'+seed+'.csv'
    else:
        pathname = global_path + 'golden-decomposing-by-' + backbone_name+'/'+ph+'dev-'+modelname+'_'+datasetname+'_'+str(gpu_id)+'-'+str(total_gpu)+'_'+seed+'.csv'
    ...
    for context_id, annotation in inf.items():
        ...
        for qa_pair in annotation["qa_pairs"]:
            ...
            if count >= st + recover_num and count < en and query_id not in used:
                subqs = model(context, question, backbone)
...
def run_dropsanity4(model, modelname, datasetname, seed, backbone, subset=-1, qdname='gpt-3.5-turbo', batch=1, whether_train=False, gpu_id=0, total_gpu=1):
    ...
    if isinstance(backbone, str):
        ...
        backbone_name = backbone[2]
    res_path = global_path + backbone_name+'-results/'
    if not os.path.exists(res_path):
        os.makedirs(res_path)
    if not os.path.exists(global_path+'golden-decomposing-by-'+qdname):
        os.makedirs(global_path+'golden-decomposing-by-'+qdname)
    ...
    #else:
    pathname = global_path + backbone_name+'-results/'+ ph + qdname + '-' + modelname + '_cot_decompose_drop_' + str(gpu_id) + '-' + str(total_gpu) + '_' + seed + '.csv'
    decomposor_path = os.path.join(global_path + 'golden-decomposing-by-' + qdname, 'full_dev-cot_decompose_drop' + '_0-1_'+seed+'.csv')
    ...
    if not os.path.exists(decomposor_path):
        for query_id, org_q in full_question.items():
            if count >= st and count < en and query_id not in used:
                preds = model(full_context[query_id], org_q.replace('</s>', ''), '', backbone)
                ...
                print('=' * 30)
                print('question: ', org_q.replace('</s>', ''))
                preds = purify_drop_ans(preds)
    ...
    with open(decomposor_path) as csvfile:
        ...
        for row in cr:
            ...
            if count >= st and count < en and query_id not in used:
                ...
                if 'mistral' in qdname:
                    subqs = mistral_extract_subqs(subqs.split('ASSISTANT: ')[-1]).strip()
                else:
                    subqs = mistral_extract_subqs(subqs).strip()
                #print(subqs)
...
def decompose_gsm8k(model, modelname, datasetname, seed, backbone, subset=-1, whether_train=False, gpu_id=0, total_gpu=1):
    ...
    pathname = global_path + 'golden-decomposing-by-' + backbone_name+'/'+ph+modelname+'_'+datasetname+'_'+str(gpu_id)+'-'+str(total_gpu)+'_'+seed+'.csv'
    print(pathname)
    ...
    for line in inf:
        ...
        if count >= st and count < en and q not in used:
            subqs = model(q, backbone).replace(',', '').strip()
            print('*' * 50)
            print(q)
            print('-' * 20)
            print(subqs)
...
def batch_decompose_gsm8k(model, modelname, datasetname, seed, backbone, subset=-1, recover_num=-1, batch=1):
    if subset != -1:
        if isinstance(backbone, str):
            ...
            backbone_name = backbone[2]
        pathname = global_path + 'golden-decomposing-by-' + backbone_name+'/'+modelname+'_'+datasetname+'_'+str(subset)+'_'+seed+'.csv'
    else:
        pathname = global_path + 'golden-decomposing-by-' + backbone+'/'+modelname+'_'+datasetname+seed+'.csv'
...
def decompose_bamboogle(model, modelname, datasetname, seed, backbone, subset=-1, whether_train=False, gpu_id=0, total_gpu=1):
    inf = open('./datasets/bamboogle.csv')
    total = 125
    if subset != -1:
        total = subset
    each_gpu_num = total // total_gpu
    ...
    for row in cr:
        q, a = row['Question'], row['Answer'].replace(',', '').replace('\"', '').strip()
...
def mistral_extract_subqs(subqs):
    #print('originial subqs: ', subqs)
    final = []
    for line in subqs.strip().split('\n'):
...
def purify_preds(anss, preds):
    ...
    for idx in range(len(preds)):
        ...
        pred = preds[idx].lower().split('the answer is')[-1].replace(':', '')
...
def run_bambooglesanity4(model, modelname, datasetname, seed, backbone, subset=-1, qdname='gpt-3.5-turbo', batch=1, whether_train=False, gpu_id=0, total_gpu=1):
    if subset == -1:
        ph = 'full_'
    else:
        ph = str(subset) + '_'
    ...
    if not os.path.exists(global_path + 'golden-decomposing-by-' + qdname+'/'):
        os.mkdir(global_path + 'golden-decomposing-by-' + qdname+'/')
    if not os.path.exists(global_path + backbone_name+'-results/'):
        os.mkdir(global_path + backbone_name+'-results/')

    pathname = global_path + backbone_name+'-results/'+ ph + qdname + '-' + modelname + '_cot_decompose_bamboogle_' + str(gpu_id) + '-' + str(total_gpu) + '_' + seed + '.csv'
    #decomposor_path = 'golden-decomposing-by-' + qdname + '/' + ph + modelname + '_' + datasetname + '_' + str(gpu_id)+'-'+str(total_gpu)+'_'+seed+'.csv'
    decomposor_path = global_path + 'golden-decomposing-by-' + qdname + '/' + 'full_train-bamboogle_cot_decompose_bamboogle_0-1_0620ep5d2' + '.csv'
    # full_dev-gsm_cot_decompose_gsm8k_0-8_0123.csv
    ...
    cr = csv.DictReader(inf)
    for row in cr:
        tmp[row['Question']] = row['Answer'].replace(',', '').replace('\"', '').strip()
    ...
    if not os.path.exists(decomposor_path):
        for org_q, a in tmp.items():
            ...
            if count >= recover_num and count < en and q not in used:
                ...
                print(a)
                print('-' * 15)
                preds = model('', q, '', backbone).replace(',', '').strip()
                purified_preds = preds.split("The answer is: ")[-1].replace(',', '').replace('\"', '')
                tp += a in purified_preds
                tt += 1
                print(f"{tp} / {tt} = {tp / (tt)}")
                print('recover_num: ', count+1)
                writer.writerow({'Question': org_q, 'Golden': a, 'subqs': ' ', 'predict': preds})
            count += 1
            if count >= st + recover_num:
                print('recover_num: ', count)
            if count >= en:
    ...
    with open(decomposor_path) as csvfile:
        ...
        for row in cr:
            ...
            if count >= st + recover_num and count < en and org_q not in used:
                ...
                if count != subset:
                    preds = model(org_q, subqs, backbone)

                    #judge, purified_preds = purify_preds(golden_a, preds)
                    purified_preds = preds.split("The answer is: ")[-1].replace(',', '').replace('\"', '')
                    print(purified_preds)
                    tp += golden_a in purified_preds
                    tt += 1
                    print(f"{tp} / {tt} = {tp / (tt)}")
                    print('recover_num: ', count+1)
                    writer.writerow({'Question': org_q, 'Golden': golden_a, 'subqs': subqs, 'predict': preds})
...
def run_gsm8ksanity4(model, modelname, datasetname, seed, backbone, subset=-1, qdname='gpt-3.5-turbo', batch=1, whether_train=False, gpu_id=0, total_gpu=1):
    ...
    pathname = global_path + backbone_name+'-results/'+ ph + qdname + '-' + modelname + '_cot_decompose_gsm8k_' + str(gpu_id) + '-' + str(total_gpu) + '_' + seed + '.csv'
    #decomposor_path = 'golden-decomposing-by-' + qdname + '/' + ph + modelname + '_' + datasetname + '_' + str(gpu_id)+'-'+str(total_gpu)+'_'+seed+'.csv'
    decomposor_path = global_path + 'golden-decomposing-by-' + qdname + '/' + 'full_dev-gsm_cot_decompose_gsm8k_0-1_0123' + '.csv'
    # full_dev-gsm_cot_decompose_gsm8k_0-8_0123.csv
    #decomposor_path = "golden-decomposing-by-gpt-3.5-turbo/full_train-gsm_cot_decompose_gsm8k_0-1_0123.csv"
            ...
            if count >= st + recover_num and count < en and org_q not in used:
                ...
                if len(org_qs) == batch or count == subset:
                    if batch != 1:
                        ...
                        preds = [model(org_qs[0], subqss[0], backbone)]
...
def load_mistral_model(path, backbonename):
    ...
    chatbot = pipeline("text-generation", model=path, device=0)
    return (chatbot, None, backbonename)
...
if __name__ == '__main__':
    ...
    print(args.model)
    model_dict = {'vinilla_ask': vinilla_ask, 'vanilla_ask': vanilla_ask, 'self_ask': self_ask, 'least_to_most_1_shot': least_to_most_1_shot, 'decompose': decompose, 'decompose2': decompose2, 'decompose3': decompose3, 'cot_1_shot': cot_1_shot, 'cot_4_shot': cot_4_shot, 'direct_decompose': direct_decompose, 'cot_decompose': cot_decompose, 'gsm_cot_decompose': gsm_cot_decompose, 'self_ask_decompose': self_ask_decompose, 'cot_qd': cot_qd, 'cot_infer_0': cot_infer_0, 'gsm_cot_infer_0': gsm_cot_infer_0, 'newgsm_cot_infer_0': newgsm_cot_infer_0, 'cot_infer_1': cot_infer_1, 'cot_infer_cot': cot_infer_cot, 'vinilla_mistral': vinilla_mistral, 'direct_qa': direct_qa, 'gsm_direct_qa': gsm_direct_qa, 'sft_qa': sft_qa, 'cot': cot, 'bamboogle_direct_qa': bamboogle_direct_qa, 'bamboogle_direct_infer_0': bamboogle_direct_infer_0, 'bamboogle_cot_decompose': bamboogle_cot_decompose}
    ...
    if args.dataset == 'drop':
        ...
        if args.backbone == 'vinilla_mistral':
            decompose_drop(model, args.model, args.dataset, args.seed, load_mistral_model('mistralai/Mistral-7B-Instruct-v0.3', args.backbone), args.decompose)
        elif '_qd-0shot_mistral' in args.backbone or '_qd-1shot_mistral' in args.backbone:
            decompose_drop(model, args.model, args.dataset, args.seed, load_mistral_model(args.backbone_path, args.backbone), args.subset, args.whether_train, args.numerical_only, args.gpu_id, args.total_gpu)
        else:
        ...
        #    if args.backbone == 'vinilla_mistral':
        #        run_drop(model, args.model, args.dataset, args.seed, load_mistral_model('mistralai/Mistral-7B-Instruct-v0.3', args.backbone), args.subset, args.decompose)
        #    elif '_qd-0shot_mistral' in args.backbone or '_qd-1shot_mistral' in args.backbone:
        #        run_drop(model, args.model, args.dataset, args.seed, load_mistral_model(args.backbone_path, args.backbone), args.subset, args.decompose)
        #    else:
    ...
    if args.dataset == 'gsm8k':
        ...
        if args.backbone == 'vinilla_mistral':
            decompose_gsm8k(model, args.model, args.dataset, args.seed, load_mistral_model('mistralai/Mistral-7B-Instruct-v0.3', args.backbone), args.subset, args.whether_train, args.gpu_id, args.total_gpu)
        elif '_qd-0shot_mistral' in args.backbone or '_qd-1shot_mistral' in args.backbone:
            decompose_gsm8k(model, args.model, args.dataset, args.seed, load_mistral_model(args.backbone_path, args.backbone), args.subset, args.whether_train, args.gpu_id, args.total_gpu)
        else:
    ...
    if args.dataset == 'bamboogle':
        if args.backbone == 'vinilla_mistral':
            decompose_bamboogle(model, args.model, args.dataset, args.seed, load_mistral_model('mistralai/Mistral-7B-Instruct-v0.3', args.backbone), args.subset, args.whether_train, args.gpu_id, args.total_gpu)
        elif '_qd-0shot_mistral' in args.backbone or '_qd-1shot_mistral' in args.backbone:
            decompose_bamboogle(model, args.model, args.dataset, args.seed, load_mistral_model(args.backbone_path, args.backbone), args.subset, args.whether_train, args.gpu_id, args.total_gpu)
        else:
            decompose_bamboogle(model, args.model, args.dataset, args.seed, args.backbone, args.subset, args.whether_train, args.gpu_id, args.total_gpu)

    if args.dataset == 'gsm8ksanity4':
        if args.backbone == 'vinilla_mistral':
            run_gsm8ksanity4(model, args.model, args.dataset, args.seed, load_mistral_model('mistralai/Mistral-7B-Instruct-v0.3', args.backbone), args.subset, args.qd, args.batch, args.whether_train, args.gpu_id, args.total_gpu)
        elif '_qd-0shot_mistral' in args.backbone or '_qd-1shot_mistral' in args.backbone:
            run_gsm8ksanity4(model, args.model, args.dataset, args.seed, load_mistral_model(args.backbone_path, args.backbone), args.subset, args.qd, args.batch, args.whether_train, args.gpu_id, args.total_gpu)
        else:
    ...
    if args.dataset == 'dropsanity4':
        if args.backbone == 'vinilla_mistral':
            run_dropsanity4(model, args.model, args.dataset, args.seed, load_mistral_model('mistralai/Mistral-7B-Instruct-v0.3', args.backbone), args.subset, args.qd, args.batch, args.whether_train, args.gpu_id, args.total_gpu)
        elif '_qd-0shot_mistral' in args.backbone or '_qd-1shot_mistral' in args.backbone:
            run_dropsanity4(model, args.model, args.dataset, args.seed, load_mistral_model(args.backbone_path, args.backbone), args.subset, args.qd, args.batch, args.whether_train, args.gpu_id, args.total_gpu)
        else:
            run_dropsanity4(model, args.model, args.dataset, args.seed, args.backbone, args.subset, args.qd, args.batch, args.whether_train)

    if args.dataset == 'bambooglesanity4':
        if args.backbone == 'vinilla_mistral':
            run_bambooglesanity4(model, args.model, args.dataset, args.seed, load_mistral_model('mistralai/Mistral-7B-Instruct-v0.3', args.backbone), args.subset, args.qd, args.batch, args.whether_train, args.gpu_id, args.total_gpu)
        elif '_qd-0shot_mistral' in args.backbone or '_qd-1shot_mistral' in args.backbone:
            run_bambooglesanity4(model, args.model, args.dataset, args.seed, load_mistral_model(args.backbone_path, args.backbone), args.subset, args.qd, args.batch, args.whether_train, args.gpu_id, args.total_gpu)
        else:
            run_bambooglesanity4(model, args.model, args.dataset, args.seed, args.backbone, args.subset, args.qd, args.batch, args.whether_train)
